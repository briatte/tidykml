...
#' 
#' If the source ends in \code{.kmz}, \code{kml_read} will treat it as a 
#' \code{.zip} file and will try to read its first file using 
#' \link[base:unz]{unz}.
...
kml_read <- function(x, ...) {
  ...
  if (is.character(x) && str_detect(x, "\\.kmz$")) {
